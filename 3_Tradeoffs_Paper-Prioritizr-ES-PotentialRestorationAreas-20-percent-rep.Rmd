---
title: "2. Seagrass Restoration Trade-offs Paper - Using PrioritizR to identify potential areas for restoration that optimizes multiple ecosystem service outcomes (20% representation)"
...
# create targets 
target_ES1 <- rep(0.20,1)
target_ES2 <- rep(0.20,2)
target_ES3 <- rep(0.20,3)
target_ES4 <- rep(0.20,4)
target_ES5 <- rep(0.20,5)
```

# FYI - 1 = Nursery Habitat, 2 = Blue Carbon, 3 = Recreation, 4 = Coastal Protection, 5 = Biodiversity

# Functionalizing a prioritization for 1 ES from Restoration
```{r}
...
Single_Service <- function(area,features,targets){
  ...
  N_ES_evaluation_output$Target <- c("0.20")
  ...
  NurseryHabitat_Rep <-sum(values(Other_ES_evaluation$NursHab,na.rm=TRUE))/sum(values(features$NursHab),na.rm=TRUE)
  BlueCarbon_Rep <-sum(values(Other_ES_evaluation$BlueCar,na.rm=TRUE))/sum(values(features$BlueCar),na.rm=TRUE)
  Recreation_Rep <-sum(values(Other_ES_evaluation$RecUse,na.rm=TRUE))/sum(values(features$RecUse),na.rm=TRUE)
  CoastalProtection_Rep <-sum(values(Other_ES_evaluation$CoastProt,na.rm=TRUE))/sum(values(features$CoastProt),na.rm=TRUE)
  Biodiversity_Rep <-sum(values(Other_ES_evaluation$Biodiv,na.rm=TRUE))/sum(values(features$Biodiv),na.rm=TRUE)
...
Single_ES_output <- Single_Service(area= PRAs_1KM,features = ESV_PRAs_1km,targets = target_ES1)

print(Single_ES_output)
# 
...
# Functionalizing a prioritization for 2 ES from Restoration
```{r}
...
Two_ES_output <- Two_Services(area= PRAs_1KM, features = ESV_PRAs_1km, targets = target_ES2)
...
# Functionalizing a prioritization for 3 ES from Restoration
```{r}
# area=Confirmed_1KM
# features=ESV_Confirmed_1km
# targets=target_ES2
...
Three_ES_output <- Three_Services(area= PRAs_1KM, features = ESV_PRAs_1km, targets = target_ES3)
print(Three_ES_output)
...
# Functionalizing a prioritization for 4 ES from Restoration
```{r}
...
Four_ES_output <- Four_Services(area= PRAs_1KM, features = ESV_PRAs_1km, targets = target_ES4)

print(Four_ES_output)
...
# Functionalizing a prioritization for 5 ES from Restoration
```{r}
# area=Confirmed_1KM
# features=ESV_Confirmed_1km
# targets=target_ES2
...
Five_ES_output <- Five_Services(area= PRAs_1KM, features = ESV_PRAs_1km, targets = target_ES5)

print(Five_ES_output)
...
Solution_Eval_Metrics_PRAs <- rbind(Single_ES_output[c(1:3,5:10)],
                               Two_ES_output,
                               Three_ES_output,
                               Four_ES_output,
                               Five_ES_output)
Solution_Eval_Metrics_PRAs$Zone <- c("Potential Restoration Areas")

write.csv(Solution_Eval_Metrics_PRAs,file = "publication_plots/Results_10_18_2023/Solution_Evaluation_Metrics_Restoration_20Percent.csv") 

Solution_Eval_Metrics_PRAs_long <- Solution_Eval_Metrics_PRAs%>%
  pivot_longer(cols = c(5:9),names_to = "Ecosystem_Services", values_to = "Representation")

Solution_Eval_Metrics_PRAs_Averaged <- Solution_Eval_Metrics_PRAs_long%>%
...
Solution_Eval_Metrics_PRAs_SUPERAVERAGED <- Solution_Eval_Metrics_PRAs_long%>%
  mutate(no_ES = ifelse(Type == "Single Service",1,ifelse(Type == "Two Services",2,ifelse(Type=="Three Services",3,ifelse(Type=="Four Services",4,5)))))%>%
...
# Solution Area
p1 <- ggplot()+
  geom_point(data=Solution_Eval_Metrics_PRAs_Averaged,aes(y=Solution_Area,x=no_ES,color=Zone))+
  geom_line(data=Solution_Eval_Metrics_PRAs_SUPERAVERAGED,aes(y=Solution_Area,x=no_ES))+
  theme_classic()+
  xlab("No of Services")+ ylab("Solution Area (sqkm)")

# Relative ES Level Held
p2 <- ggplot()+
  geom_point(data=Solution_Eval_Metrics_PRAs_Averaged,aes(y=Mean_Representation,x=no_ES,color=Zone))+
  geom_line(data=Solution_Eval_Metrics_PRAs_SUPERAVERAGED,aes(y=Mean_Representation,x=no_ES))+
  theme_classic()+
  xlab("No of Services")+ ylab("Avg ES Representation (%)")

tiff(filename = "publication_plots/Results_10_18_2023/Fig_20perc_solutions_PRAs.tif",width = 7.5,height = 4,units = "in",res = 300)
p1 + p2 & theme_classic() & theme(legend.position = "none") & plot_annotation(title = "Restoration Areas (20% representation)")
