---
title: "2. Seagrass Restoration Trade-offs Paper - Using PrioritizR to identify potential areas for conservation that optimizes multiple ecosystem service outcomes (20% represenation)"
...
# create targets 
target_ES1 <- rep(0.2,1)
target_ES2 <- rep(0.2,2)
target_ES3 <- rep(0.2,3)
target_ES4 <- rep(0.2,4)
target_ES5 <- rep(0.2,5)
```
...
Solution_Eval_Metrics_Confrimed_Beds <- rbind(Single_ES_output[c(1:3,5:10)],
                               Two_ES_output,
                               Three_ES_output,
                               Four_ES_output,
                               Five_ES_output)
...
write.csv(Solution_Eval_Metrics_Confrimed_Beds,file = "publication_plots/Results_10_18_2023/Solution_Evaluation_Metrics_Confirmed_Beds_20Percent.csv") 
...
tiff(filename = "publication_plots/Results_10_18_2023/Fig_20perc_solutions_ConfirmedBeds.tif",width = 7.5,height = 4,units = "in",res = 300)
p1 + p2 & theme_classic() & theme(legend.position = "none") & plot_annotation(title = "Confirmed Beds (20% representation)")
dev.off()
