...
ES_Zscores_1HA_75th_Confirmed_df<-ES_Zscores_1HA_df%>%
  filter(Area=="Confirmed")%>%
  mutate(BioDiv_percentile=pnorm(BioDiv),NurHab_percentile=pnorm(NurHab),BlueCar_percentile=pnorm(BlueCar),RecTour_percentile=pnorm(RecTour),CoastProt_percentile=pnorm(Expos))%>%
  mutate(BioDiv_QT=ifelse(BioDiv>=0.75,1,0))%>%
  mutate(NurHab_QT=ifelse(NurHab>=0.75,1,0))%>%
  mutate(BlueCar_QT=ifelse(BlueCar>=0.75,1,0))%>%
  mutate(RecTour_QT=ifelse(RecTour>=0.75,1,0))%>%
  mutate(CoastProt_QT=ifelse(Expos>=0.75,1,0))%>%
  dplyr::select(BioDiv_QT:CoastProt_QT)%>%
  mutate(Bio_Nurs_WW=ifelse(BioDiv_QT==1 & NurHab_QT==1,1,0))%>%
  mutate(Bio_Blu_WW=ifelse(BioDiv_QT==1 & BlueCar_QT==1,1,0))%>%
  mutate(Bio_Rec_WW=ifelse(BioDiv_QT==1 & RecTour_QT==1,1,0))%>%
...
ES_Zscores_5KM_75th_Confirmed_df<-ES_Zscores_5KM_df%>%
  filter(Area=="Confirmed")%>%
  mutate(BioDiv_percentile=pnorm(BioDiv),NurHab_percentile=pnorm(NurHab),BlueCar_percentile=pnorm(BlueCar),RecTour_percentile=pnorm(RecTour),CoastProt_percentile=pnorm(Expos))%>%
  mutate(BioDiv_QT=ifelse(BioDiv>=0.75,1,0))%>%
  mutate(NurHab_QT=ifelse(NurHab>=0.75,1,0))%>%
...
Winwin_frequency_1HA_75th_Confirmed_summed <- ES_Zscores_1HA_75th_Confirmed_df%>%
  dplyr::select(Bio_Nurs_WW:Rec_Coast_WW)%>%
  summarize_all(.funs = sum)
#
Winwin_Fraction_1HA_75th_Confrimed<-Winwin_frequency_1HA_75th_Confirmed_summed/dim(ES_Zscores_1HA_75th_Confirmed_df)[[1]]

write.csv(Winwin_Fraction_1HA_75th_Confrimed,file="analytical_output/Win_Win_Fraction_75th_Percentile_ConfirmedBeds_1HA.csv")

Winwin_frequency_5KM_75th_Confirmed_summed <- ES_Zscores_5KM_75th_Confirmed_df%>%
  dplyr::select(Bio_Nurs_WW:Rec_Coast_WW)%>%
  summarize_all(.funs = sum)
#
Winwin_Fraction_5KM_75th_Confrimed<-Winwin_frequency_5KM_75th_Confirmed_summed/dim(ES_Zscores_5KM_75th_Confirmed_df)[[1]]

write.csv(Winwin_Fraction_5KM_75th_Confrimed,file="analytical_output/Win_Win_Fraction_75th_Percentile_ConfirmedBeds_5KM.csv")
...
ES_Zscores_5KM_75th_PRAs_df<-ES_Zscores_5KM_df%>%
  filter(Area=="Unconfirmed")%>%
  mutate(BioDiv_percentile=pnorm(BioDiv),NurHab_percentile=pnorm(NurHab),BlueCar_percentile=pnorm(BlueCar),RecTour_percentile=pnorm(RecTour),CoastProt_percentile=pnorm(Expos))%>%
  mutate(BioDiv_QT=ifelse(BioDiv>=0.75,1,0))%>%
  mutate(NurHab_QT=ifelse(NurHab>=0.75,1,0))%>%
...
ES_Zscores_1HA_75th_PRAs_df<-ES_Zscores_1HA_df%>%
  filter(Area=="Unconfirmed")%>%
  mutate(BioDiv_percentile=pnorm(BioDiv),NurHab_percentile=pnorm(NurHab),BlueCar_percentile=pnorm(BlueCar),RecTour_percentile=pnorm(RecTour),CoastProt_percentile=pnorm(Expos))%>%
  mutate(BioDiv_QT=ifelse(BioDiv>=0.75,1,0))%>%
  mutate(NurHab_QT=ifelse(NurHab>=0.75,1,0))%>%
...
Winwin_frequency_1HA_75th_PRAs_summed <- ES_Zscores_1HA_75th_PRAs_df%>%
  dplyr::select(Bio_Nurs_WW:Rec_Coast_WW)%>%
  summarize_all(.funs = sum)
# 
Winwin_Fraction_1HA_75th_PRAs<-Winwin_frequency_1HA_75th_PRAs_summed/dim(ES_Zscores_1HA_75th_PRAs_df)[[1]]

write.csv(Winwin_Fraction_1HA_75th_PRAs,file="analytical_output/Win_Win_Fraction_75th_Percentile_PRAs_1HA.csv")

Winwin_frequency_5KM_75th_PRAs_summed <- ES_Zscores_5KM_75th_PRAs_df%>%
  dplyr::select(Bio_Nurs_WW:Rec_Coast_WW)%>%
  summarize_all(.funs = sum)
# 
Winwin_Fraction_5KM_75th_PRAs<-Winwin_frequency_5KM_75th_PRAs_summed/dim(ES_Zscores_5KM_75th_PRAs_df)[[1]]

write.csv(Winwin_Fraction_5KM_75th_PRAs,file="analytical_output/Win_Win_Fraction_75th_Percentile_PRAs_5KM.csv")
